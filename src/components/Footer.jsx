...
const SOCIALS = [
  ...
  {
    name: 'LinkedIn', href: 'https://www.linkedin.com/in/j-saravia-5bb36637b/',
  ...
  {
    name: 'Instagram', href: 'https://www.instagram.com/js.arvx?igsh=aWM0eW5zNGRyM3R2',
    icon: <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><rect x="2" y="2" width="20" height="20" rx="5" ry="5"/><path d="M16 11.37A4 4 0 1 1 12.63 8 4 4 0 0 1 16 11.37z"/><line x1="17.5" y1="6.5" x2="17.51" y2="6.5"/></svg>,
  },
  {
      ...
      <div className="footer-inner">
        ...
        <div className="footer-top">
          ...
          <div className="footer-contact-btns">
            {[{ id: 'whatsapp', label: 'WhatsApp' }, { id: 'email', label: 'Correo' }].map(c => (
              <button key={c.id} className="footer-contact-btn" onClick={() => onOpenModal(c.id)}>
                {c.label}
              </button>
            ))}
            <button className="footer-colab-btn" onClick={() => onOpenModal('colaboradores')}>
              Colaboradores
            </button>
          </div>
        </div>
        ...
        <div className="footer-bottom">
          <p className="footer-copy">
            © {new Date().getFullYear()} JC Saravia · Todos los derechos reservados
          </p>
          <div className="footer-socials">
            {SOCIALS.map(s => (
              <a key={s.name} href={s.href} target="_blank" rel="noreferrer"
                title={s.name} className="footer-social">
                {s.icon}
              </a>
            ))}
          </div>
